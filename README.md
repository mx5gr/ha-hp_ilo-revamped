...
A drop-in replacement for the [official HP iLO integration](https://www.home-assistant.io/integrations/hp_ilo/) with significant improvements, supporting **iLO 3, 4, 5, and 6** (HP/HPE ProLiant G7 through Gen11):
...
# iLO Generation Compatibility

The integration auto-detects the iLO generation on first connect via `GET_FW_VERSION` and normalises any structural differences in the RIBCL response data transparently. No manual configuration is needed.

| iLO Generation | HP/HPE Servers | RIBCL Protocol | Status |
|---|---|---|---|
| **iLO 3** | ProLiant G7 and earlier (BL/DL/ML) | ✅ | ✅ Supported |
| **iLO 4** | ProLiant Gen8, Gen9 | ✅ | ✅ Supported (primary test target) |
| **iLO 5** | ProLiant Gen10, Gen10 Plus | ✅ | ✅ Supported |
| **iLO 6** | ProLiant Gen11 | ✅ | ✅ Supported |

### Feature availability by generation

| Feature | iLO 3 | iLO 4 | iLO 5 | iLO 6 |
|---|:---:|:---:|:---:|:---:|
| Temperature / Fan / Power sensors | ✅ | ✅ | ✅ | ✅ |
| Processor sensors | ✅ | ✅ | ✅ | ✅ |
| Memory DIMM sensors | ✅ | ✅ | ✅ | ✅ |
| NIC sensors | ✅ | ✅ | ✅ | ✅ |
| Health-at-a-Glance sensors | ✅ | ✅ | ✅ | ✅ |
| Firmware information sensors | ✅ | ✅ | ✅ | ✅ |
| Event log sensors | ✅ | ✅ | ✅ | ✅ |
| Power control (switch + buttons) | ✅ | ✅ | ✅ | ✅ |
| Storage controller sensors | — ¹ | ✅ | ✅ | ✅ |
| BIOS/Hardware aggregate sensor | — ² | ✅ | ✅ | ✅ |
| Power Saver mode sensor | ✅ | ✅ | — ³ | — ³ |
| Power Cap / pwreg sensors | ✅ | ✅ | — ³ | — ³ |
| Critical Temp Remain Off binary sensor | — ³ | ✅ | — ³ | — ³ |

¹ iLO 3 blade servers (BL-series) have no HP Smart Array controller exposed via RIBCL; rack/tower G7 servers with an SA controller will surface storage sensors normally.  
² `BIOS_HARDWARE` health-at-a-glance subsystem was introduced in iLO 4.  
³ These RIBCL commands raise `IloError` on this generation. The sensors simply do not appear in the HA UI — no errors are logged.

### Known per-generation data differences (handled automatically)

| Difference | iLO 3 | iLO 4 | iLO 5 | iLO 6 |
|---|---|---|---|---|
| Sensor values (temp, fan, power) | `(value, unit)` tuple | `(value, unit)` tuple | plain `int`/`float` | plain `int`/`float` |
| `nic_information` shape | dict, no IP/location keys | dict with `location` + `ip_address` | list (normalised to dict) | list (normalised to dict) |
| `storage` shape | dict or absent | dict | list (normalised to dict) | list (normalised to dict) |
| `memory` shape | `memory_components` list | `memory_components` list | `memory_details` dict (normalised) | `memory_details` dict (normalised) |
| `network_settings` IP key | `ip_address` | `ip_address` | `ipv4_address` (mirrored) | `ipv4_address` (mirrored) |
| `processors` dict has `status` field | No | Yes | Yes | Yes |

All of the above are handled in `coordinator.py`'s normalisation layer before data reaches any sensor entity.
...
Processor sensors missing on iLO 3 | Fixed guard — iLO 3 processors have no `status` field; absence now treated as OK rather than Unknown
...
<h2>Tested On</h2>
<ul>
<li>HP ProLiant BL460c G7 — iLO 3 firmware <code>1.82</code> (verified against python-hpilo fixture data)</li>
<li>HP ProLiant MicroServer Gen8 — iLO 4 firmware <code>2.82 Feb 06 2023</code></li>
<li>Intel Xeon E3-1220 V2, mixed DDR3 DIMMs</li>
<li>Home Assistant 2026.2.3 with <code>python-hpilo 4.4.3</code></li>
</ul>
