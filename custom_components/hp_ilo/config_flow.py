...
from homeassistant.const import (
    CONF_HOST, CONF_NAME, CONF_DESCRIPTION, ATTR_CONFIGURATION_URL,
    CONF_PORT, CONF_PROTOCOL, CONF_UNIQUE_ID, CONF_USERNAME, CONF_PASSWORD,
)
from .sensor import SENSOR_TYPES, DOMAIN
from .const import ILO_GEN_UNKNOWN, ILO_GEN_3

_LOGGER = logging.getLogger(__name__)


def _detect_ilo_gen_from_fw(fw_version: dict | None) -> int:
    """Parse management_processor field and return numeric iLO generation.

    Duplicated here (rather than imported from coordinator) to avoid a
    circular import — config_flow imports sensor, coordinator imports const.

    iLO 3 (G7 and earlier) returns "iLO3", iLO 4 returns "iLO4", etc.
    """
    if not fw_version:
...
class HpIloFlowHandler(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_ssdp(self, discovery_info: SsdpServiceInfo) -> FlowResult:
        ...
        # Extra security: reject non-iLO SSDP announcements from HPE devices.
    ...
    async def async_step_confirm(self, user_input=None):
        ...
        data_schema = {
            ...
            vol.Required(CONF_PORT, default=self.config.get(CONF_PORT, "")): vol.All(
    ...
    async def async_step_user(self, user_input=None):
        ...
        if user_input is not None:
            ...
            self.config["ilo_gen"] = ILO_GEN_UNKNOWN
    ...
    async def async_step_auth(self, user_input=None, errors=None):
        """Authenticate to the device and detect the iLO generation."""
        errors_dict = {}
        ...
        if user_input is not None:
            try:
                ...
                # get_fw_version() is reliable across all iLO versions and is
                # the same call the coordinator makes, so there is no extra
                # round-trip at runtime.
                try:
                    fw = self.ilo.get_fw_version()
                    detected_gen = _detect_ilo_gen_from_fw(fw)
                    self.config["ilo_gen"] = detected_gen
                    mp = fw.get("management_processor", "unknown")
                    _LOGGER.info(
                        "Detected %s (generation %s) at %s",
                        mp, detected_gen, self.config[CONF_HOST],
                    )
                except Exception as fw_err:
                    _LOGGER.warning(
                        "Could not detect iLO generation for %s: %s",
                        self.config[CONF_HOST], fw_err,
                    )
                    self.config["ilo_gen"] = ILO_GEN_UNKNOWN

                # ── Get serial number for stable unique_id ─────────────────
                try:
                    ...
                    if host_data and len(host_data) > 0:
                        ...
                        if serial_number:
                            self.config[CONF_UNIQUE_ID] = (
                                f"{self.config[CONF_HOST]}_{serial_number}"
                            )
                except Exception as hd_err:
                    _LOGGER.error(
                        "Failed to get host data from iLO at %s: %s",
                        self.config[CONF_HOST], hd_err,
                    )
                ...
                _LOGGER.warning(
                    "Authentication failed for %s: %s", self.config[CONF_HOST], error
                )
                errors_dict["base"] = "invalid_auth"
            except hpilo.IloCommunicationError as error:
                _LOGGER.warning(
                    "Communication error with %s: %s", self.config[CONF_HOST], error
                )
                errors_dict["base"] = "cannot_connect"
            except (hpilo.IloError, hpilo.IloNotARackServer) as error:
                _LOGGER.error(
                    "iLO error for %s: %s", self.config[CONF_HOST], error
                )
                errors_dict["base"] = "unknown"
            except ConnectionError as error:
                _LOGGER.warning(
                    "Connection error to %s: %s", self.config[CONF_HOST], error
                )
                errors_dict["base"] = "cannot_connect"
            except OSError as error:
                _LOGGER.warning(
                    "OS error connecting to %s: %s", self.config[CONF_HOST], error
                )
                if "Name or service not known" in str(error) or "nodename nor servname" in str(error):
                    errors_dict["base"] = "invalid_host"
                else:
                    errors_dict["base"] = "cannot_connect"
            except Exception as error:
                _LOGGER.exception(
                    "Unexpected error setting up iLO for %s: %s",
                    self.config[CONF_HOST], error,
                )
                errors_dict["base"] = "unknown"
        ...
        return self.async_show_form(
            ...
            data_schema=vol.Schema(data_schema),
            errors=errors_dict,
            description_placeholders={CONF_HOST: self.config[CONF_HOST]},
        )
